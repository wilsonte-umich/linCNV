...
adjustCoord <- function(pos, expand, max){
    pos <- (pos - 1) * expand # thus all pixels of the region are VISIBLE (box masks adjacent bins)
    min(max(pos, 1), max)
}
invertY <- function(y, ymax) ymax - y + 1
addRegionBox <- function(cimg, expand, triangle, box, col){ # box=c(x0,y0,x1,y1)
    # need to handle triangle
    if(is.null(box)) return(cimg)
    
    print(paste(triangle, col))
    ...
    print(paste(x0, y0, x1, y1))
    
    
    implot(cimg, rect(x0, y0, x1, y1, border=col, lwd=1))
    #draw_rect(cimg, x0, y0, x1, y1, color=col, filled=FALSE) # filled = FALSE not working properly!
}

## helper function to judge desirability of hue adjustments
#decorateTestImage <- function(cimg, expand, N, hue){
#    margin <- round(N / 10 * expand, 0)
#    cimg <- pad(cimg, margin, 'y', pos=-1, "white") # x,-1 pads left; y,1 pads bottom; xy,0 puts half on all sides
#    implot(cimg, text(
#        dim(cimg)[1]/2, margin/2, hue,
#        adj=0.5, col=hues[[hue]]$value
#    ))
#}
#saveTestImages <- function(){
#    N <- 200
#    intensity <- matrix(seq(0, 1, length.out=N), N, N, byrow=TRUE)
#    for(hue in names(hues)) {
#        saveHeatMap_one_color(intensity, hue, paste0(hue, '.rectangle.png'), N, hue,
#                              expand=2, decorate=decorateTestImage, 
#                              triangle=FALSE)
#        saveHeatMap_one_color(intensity, hue, paste0(hue, '.triangle.png'), N, hue,
#                              expand=2, decorate=decorateTestImage, 
#                              triangle=TRUE)
#    }
#}
